...
contract RaffluxMain is IERC721Receiver, IERC1155Receiver, Ownable {
    ...
    /*//////////////////////////////////////////////////////////////
                                 EVENTS
       //////////////////////////////////////////////////////////////*/
    event Log_DelegateTicket(uint256 _raffleId, address indexed _receiver, address indexed _sender);
    event Log_BuyTicket(uint256 _raffleId, uint256 _amount, address indexed buyer);
    ...
    event Log_RefundTicket(uint256 _raffleId, address _receiver, uint256 _amount);
    ...
    modifier checkZero(uint256 _points) {
        if (_points == 0) {
            revert transactReverted("points can't be zero");
        }
        _;
    }
    ...
    //update points
    function updatePoints(address _user, uint256 _points, uint256 _raffleId, bool _bool) private checkZero(_points) {
        ...
        if (_bool) {
            ...
            if (viewPoints(_user, _raffleId) < _points) {
                revert transactReverted("not enough points available");
            }
    ...
    function depositNft(address _tokenAddress, uint256 _tokenId, uint256 _proposalId) private {
        ...
        public
    {
        ...
        emit Log_ProposeRaffle(
            _description, _owner, _startTime, _endTime, _maxTicket, _ticketPerUser, _prize, _tokenId, _price
            );
    ...
    function delegateTicket(uint256 _raffleId, address _receiver) public {
        ...
        uint256 lastTicketIdIndex =
            ticketId[_raffleId][msg.sender].length <= 1 ? 0 : ticketId[_raffleId][msg.sender].length - 1;

        // Get the index of the last buyer in the buyers array
        uint256 lastBuyerIndex = buyers[_raffleId].length <= 1 ? 0 : buyers[_raffleId].length - 1;

        //Get the last item in the array for the caller
        uint256 ticketIdIndex = ticketId[_raffleId][msg.sender][lastTicketIdIndex];

        // Check if the caller has any available tickets
        if (totalUserTicket[_raffleId][msg.sender] == 0 || maximumUserTicket[_raffleId][msg.sender] == 0) {
        ...
        if (maximumUserTicket[_raffleId][_receiver] >= raffles[_raffleId].ticketPerUser) {
            revert transactReverted("maximum ticket reached");
        }
        ...
        if (buyers[_raffleId][ticketId[_raffleId][msg.sender][lastBuyerIndex]] == msg.sender) {
    ...
    function executeRaffle(uint256 _raffleId) public {
        ...
        withdrawNft(raffles[_raffleId].prize, raffles[_raffleId].tokenId);
